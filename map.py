...
def init():
    ...
    sand = np.zeros((longueur, largeur),
                    dtype=np.uint8)  # changed variable type from float to uint8 to reduce NPY file size
    goal_x = 20
    goal_y = largeur - 20
    first_update = False
...
class CarApp(App):
    ...
    def save(self, obj):
        ...
        root.withdraw()
        # save brain
        pth_file_path = filedialog.asksaveasfilename(title='Save PTH file',
    ...
    def load(self, obj):
        ...
        root.withdraw()
        # load brain
        pth_file_path = filedialog.askopenfilename(title='Open PTH file',
                                                     filetypes=(("PTH files", "*.pth"), ("all files", "*.*")))
        if pth_file_path == ():
            pth_file_path = ""
        if pth_file_path != "":
    ...
    def mapsave(self, obj):
        ...
        # save png file
        png_file_path = filedialog.asksaveasfilename(title='Save PNG file',
        ...
        # save npy file
        npy_file_path = filedialog.asksaveasfilename(title='Save NPY file',
    ...
    def mapload(self, obj):
        ...
        # open png file
        png_file_path = filedialog.askopenfilename(title='Open PNG file',
                                                     filetypes=(("PNG files", "*.png"), ("all files", "*.*")))
        if png_file_path == ():
            png_file_path = ""
        ...
        # open npy file
        npy_file_path = filedialog.askopenfilename(title='Open NPY file',
                                                     filetypes=(("NPY files", "*.npy"), ("all files", "*.*")))
        if npy_file_path == ():
            npy_file_path = ""
